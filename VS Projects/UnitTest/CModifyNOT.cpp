...
public:
	...
	void OperTData(T tData, HEXSPAN hsp) {
		ModifyOperTData(OPER_NOT, tData, hsp);
	}
	...
	TEST_METHOD(OperUInt8) {
		...
		constexpr std::uint8_t ui8Result = static_cast<std::uint8_t>(~ui8Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt16) {
		...
		constexpr std::uint16_t ui16Result = static_cast<std::uint16_t>(~ui16Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
