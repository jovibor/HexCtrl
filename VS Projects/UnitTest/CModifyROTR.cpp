...
public:
	...
	void OperTData(T tData, HEXSPAN hsp) {
		ModifyOperTData(OPER_ROTR, tData, hsp);
	}
	...
	TEST_METHOD(OperInt8) {
		...
		constexpr std::int8_t i8Result = std::rotr(static_cast<std::uint8_t>(i8Set), i8Oper);
	...
	TEST_METHOD(OperUInt8) {
		...
		constexpr std::uint8_t ui8Result = std::rotr(ui8Set, ui8Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt16) {
		...
		constexpr std::int16_t i16Result = std::rotr(static_cast<std::uint16_t>(i16Set), i16Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt16) {
		...
		constexpr std::uint16_t ui16Result = std::rotr(ui16Set, ui16Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt32) {
		...
		constexpr std::int32_t i32Result = std::rotr(static_cast<std::uint32_t>(i32Set), i32Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt32) {
		...
		constexpr std::uint32_t ui32Result = std::rotr(ui32Set, ui32Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt64) {
		...
		constexpr std::int64_t i64Result = std::rotr(static_cast<std::uint64_t>(i64Set), i64Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt64) {
		...
		constexpr std::uint64_t ui64Result = std::rotr(ui64Set, ui64Oper);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
