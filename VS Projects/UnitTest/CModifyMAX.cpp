...
public:
	...
	void OperTData(T tData, HEXSPAN hsp) {
		ModifyOperTData(OPER_MAX, tData, hsp);
	}
	...
	TEST_METHOD(OperInt8) {
		...
		constexpr std::int8_t i8Result = i8Oper;
	...
	TEST_METHOD(OperUInt8) {
		...
		constexpr std::uint8_t ui8Result = ui8Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt16) {
		...
		constexpr std::int16_t i16Result = i16Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt16) {
		...
		constexpr std::uint16_t ui16Result = ui16Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt32) {
		...
		constexpr std::int32_t i32Result = i32Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt32) {
		...
		constexpr std::uint32_t ui32Result = ui32Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt64) {
		...
		constexpr std::int64_t i64Result = i64Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt64) {
		...
		constexpr std::uint64_t ui64Result = ui64Oper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperFloat) {
		...
		constexpr float flOper = 123.8765432F;
		constexpr float flResult = flOper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperDouble) {
		...
		constexpr double dblOper = 1234.8765432;
		constexpr double dblResult = dblOper;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
