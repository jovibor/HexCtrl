...
private:
	static constexpr auto m_iRangeToTest = 16; //How many elements of given type to test.
	IHexCtrl* m_pHex { GetHexCtrl() };
public:
	template<typename T>
	void OperTData(T tData, HEXSPAN hsp) {
		ModifyOperTData(OPER_BITREV, tData, hsp);
	}
	...
	TEST_METHOD(OperInt8) {
		...
		constexpr std::int8_t i8Result = BitReverse(i8Set);
	...
	TEST_METHOD(OperUInt8) {
		...
		constexpr std::uint8_t ui8Result = BitReverse(ui8Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt16) {
		...
		constexpr std::int16_t i16Result = BitReverse(i16Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt16) {
		...
		constexpr std::uint16_t ui16Result = BitReverse(ui16Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt32) {
		...
		constexpr std::int32_t i32Result = BitReverse(i32Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt32) {
		...
		constexpr std::uint32_t ui32Result = BitReverse(ui32Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt64) {
		...
		constexpr std::int64_t i64Result = BitReverse(i64Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt64) {
		...
		constexpr std::uint64_t ui64Result = BitReverse(ui64Set);
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
