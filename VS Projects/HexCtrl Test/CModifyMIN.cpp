...
public:
	...
	void OperTData(T tData, HEXSPAN hsp) {
		ModifyOperTData(OPER_MIN, tData, hsp);
	}
	...
	TEST_METHOD(OperInt8) {
		...
		constexpr std::int8_t i8Result = i8Set;
	...
	TEST_METHOD(OperUInt8) {
		...
		constexpr std::uint8_t ui8Result = ui8Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt16) {
		...
		constexpr std::int16_t i16Result = i16Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt16) {
		...
		constexpr std::uint16_t ui16Result = ui16Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt32) {
		...
		constexpr std::int32_t i32Result = i32Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt32) {
		...
		constexpr std::uint32_t ui32Result = ui32Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperInt64) {
		...
		constexpr std::int64_t i64Result = i64Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperUInt64) {
		...
		constexpr std::uint64_t ui64Result = ui64Set;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperFloat) {
		...
		constexpr float flResult = flSet;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
	...
	TEST_METHOD(OperDouble) {
		...
		constexpr double dblResult = dblSet;
		const HEXSPAN hss { 0, m_pHex->GetDataSize() };
